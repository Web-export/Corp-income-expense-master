...
class Header extends React.Component {
  handleLeftPress = () => {
    ...
    return back ? navigation.openDrawer() : navigation.goback();
  };
  renderRight = () => {
  ...
  render() {
    ...
    const { routeName } = navigation.state;
    const noShadow = ['Search', 'Categories', 'Deals', 'Pro'].includes(routeName);
    const headerStyles = [
      !noShadow ? styles.shadow : null,
      transparent ? { backgroundColor: '#386389' } : null
    ];
    ...
    return (
      <Block style={headerStyles}>
        <NavBar
          ...
          left={
            back? (<Icon hide={false}
              name={back ? 'align-left-22x' : 'minimal-left2x'}
              family="NowExtra"
              size={16}
